...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      split_e = tf.split(1, self.config.num_steps, embeddings)    
      for e in split_e:
        inputs.append(tf.squeeze(e, squeeze_dims=[1]))
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope('projection_layer') as projscope:
      U = tf.get_variable("U", (self.config.hidden_size, len(self.vocab)),dtype=tf.float32)
      b_2 = tf.Variable(tf.zeros((len(self.vocab),)), name="b_2", dtype=tf.float32)
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as RNNscope:
      ...
      for step in np.arange(self.config.num_steps):
        input_dropout = tf.nn.dropout(inputs[step], self.dropout_placeholder)
        #if step > 0 :
        #  RNNscope.reuse_variables()
        z = tf.matmul(prev_h, H) + tf.matmul(input_dropout, I) + b_1
        current_h = tf.nn.sigmoid(z)
        rnn_output_dropout = tf.nn.dropout(current_h, self.dropout_placeholder)
        rnn_outputs.append(rnn_output_dropout)
        prev_h = current_h
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    curr_input = np.array(tokens[-1]).reshape(1,1)
    feed = {
              model.input_placeholder: curr_input,

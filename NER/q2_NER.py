...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      with tf.variable_scope('EmbeddingLayer') as varscope:
        L = tf.Variable(tf.convert_to_tensor(self.wv, dtype=tf.float32), name="Embedding")
      window = tf.reshape(tf.nn.embedding_lookup(L, self.input_placeholder),(-1, self.config.window_size*self.config.embed_size))
      ### END YOUR CODE
      return window
  ...
  def add_loss_op(self, y):
    ...
    ### YOUR CODE HERE
    #data loss
    y = y - tf.reduce_max(y, reduction_indices=[1], keep_dims=True)
    loss_per_input = tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)
    loss = tf.reduce_mean(loss_per_input)
  ...
  def add_training_op(self, loss):
    ...
    """
    ### YOUR CODE HERE
    gdopt = tf.train.AdamOptimizer(self.config.lr)
    train_op = gdopt.minimize(loss)
    ### END YOUR CODE
    return train_op

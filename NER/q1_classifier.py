...
class SoftmaxModel(Model):
  ...
  def add_training_op(self, loss):
    ...
    """
    ### YOUR CODE HERE
    gdopt = tf.train.GradientDescentOptimizer(self.config.lr)
    train_op = gdopt.minimize(loss, var_list=tf.all_variables())
    ### END YOUR CODE
    return train_op
  ...
  def add_model(self, input_data):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope('softmax_model_layer') as varscope:
      W = tf.Variable(tf.zeros([self.config.n_features, self.config.n_classes], dtype=tf.float32), name="W")
      b = tf.Variable(tf.zeros([self.config.n_classes], dtype=tf.float32), name="b")
